...
void Misc::drawFov() noexcept
{
    ...
    if (config->misc.drawFov)
    {
        ...
        if (!config->legitbot[weaponId].enabled) weaponId = 0;
        if (!config->legitbot[weaponId].enabled) return;
        auto screenSize = interfaces->surface->getScreenSize();
        if (config->legitbot[weaponId].silent) interfaces->surface->setDrawColor(255, 10, 10, 255);
        else interfaces->surface->setDrawColor(10, 255, 10, 255);
        float radius = std::tan(degreesToRadians(config->legitbot[weaponId].fov) / 2.f) / std::tan(degreesToRadians(local->isScoped() ? local->fov() : (105 + config->visuals.fov)) / 2.f) * screenSize.first;
...
void Misc::watermark() noexcept
{
    ...
    ImGui::Text("dware | %s | %s", fps.c_str(), ping.c_str());
...
void Misc::gamesenseTag(bool tagChanged) noexcept
{
    if (config->misc.gamesenseTag) {
        static int prevtime;
        static int prevval;
        if (int(memory->globalVars->currenttime * 3.2) != prevtime)
        {
            if (prevval != int(memory->globalVars->currenttime * 3.2) % 28)
            {
                prevval = int(memory->globalVars->currenttime * 3.2) % 28;
                switch (int(memory->globalVars->currenttime * 3.2) % 28) {
                case 28: { memory->setClanTag("e               ", "e");; break; }
                case 27: { memory->setClanTag("re              ", "re");; break; }
                case 26: { memory->setClanTag("are             ", "are");; break; }
                case 25: { memory->setClanTag("ware            ", "ware");; break; }
                case 24: { memory->setClanTag("dware           ", "dware");; break; }
                case 23: { memory->setClanTag("ndware          ", "ndware");; break; }
                case 22: { memory->setClanTag("ondware         ", "ondware");; break; }
                case 21: { memory->setClanTag("mondware        ", "mondware");; break; }
                case 20: { memory->setClanTag("amondware       ", "amondware");; break; }
                case 19: { memory->setClanTag("iamondware      ", "iamondware");; break; }
                case 18: { memory->setClanTag("diamondware     ", "diamondware     ");; break; }
                case 17: { memory->setClanTag(" diamondware     ", "diamondware    ");; break; }
                case 16: { memory->setClanTag("  diamondware     ", "diamondware     ");; break; }
                case 15: { memory->setClanTag("   diamondware     ", "diamondware   ");; break; }
                case 14: { memory->setClanTag("    diamondware    ", " diamondware   ");; break; }
                case 13: { memory->setClanTag("     diamondware   ", "  diamondware   ");; break; }
                case 12: { memory->setClanTag("      diamondware  ", "   diamondware   ");; break; }
                case 11: { memory->setClanTag("      diamondware", "     diamondware");; break; }
                case 10: { memory->setClanTag("       diamondwar", "       diamondwar");; break; }
                case 9: { memory->setClanTag("         diamondwa", "         diamondwa");; break; }
                case 8: { memory->setClanTag("          diamondw", "          diamondw");; break; }
                case 7: { memory->setClanTag("           diamond", "           diamond");; break; }
                case 6: { memory->setClanTag("            diamon", "            diamon");; break; }
                case 5: { memory->setClanTag("             diamo", "             diamo");; break; }
                case 4: { memory->setClanTag("              diam", "              diam");; break; }
                case 3: { memory->setClanTag("               dia", "               dia");; break; }
                case 2: { memory->setClanTag("                di", "                di");; break; }
                case 1: { memory->setClanTag("                 d", "                 d");; break; }
