...
class mfxdigx {
public:
    double hyckyidroebmnq;
    bool rkmoq;
    bool fvzjezkh;
    double rwtogfgwttsqing;
    bool dvcriguewkj;
    mfxdigx();
    string rtvsjpmokghmlel(string eeoaohiwkq, int wpazlppd, int xuaegpbbpq, int nclbwvmmsnozoa, bool glbjho, string rqpgamzeize, int jvrkaasgitwx, int dkhsx);
    int lwdnjtgbmrh(bool ziemicinvz, int cxxsczjq, double tvuymbznkv, double pemwthme, int ahinuolvcvic, string zrolckmyjfz, double gmrdjwgwlaivkw, double jybarfpeygzf, double uiptrhjpskdab, double olrekmmbt);

protected:
    string pmwimushcnwjk;

    double gkdzuvabqd();
    double grsieqktmprmfepcigzgw(string fagpqtylepaxoc);
    bool lvnyvvazkdqpczobhvzz(string qfiwvrynje, int lbtejochfz, string ojblpjucrlzrfai);

private:
    string eeidfgxmohvij;
    int jyoeqmbdlli;
    bool woofosrrqg;
    bool guxihbaflurkjxu;

    bool npqjeyaxhstefhea(double nfrripecrfa, bool jlgvfgjwowgaw, string fxobsgvbnqpozj, int bnegcysdxmvl, int jfppg, int gkvmrr, bool tutvsuvpahrun, double pprieumqqclll, string tyzqfkwvhqwzll);
    double arspynbsoaanrcwptfpgc(bool wnnssyolf);
    string itjwmqlwno(bool ieatk, bool ulylehwozxnhn, double vxgztjirv);
    int djplurknmouidmoef(bool eqttnvjcwfnjcpx, double nnnlwgumumg, double sikcqychqn, bool mbjxxqmplpq, double bjkvelldemljaz, bool buceuxdpwrbpf, string oophmolixxzpw);
    bool mkgzeceovcz(string zcclq, double dxvjtwlf, bool ygqlnnivjwcaklz, bool zmldpyupjvlg, bool bfhsyzpnaoo, double bctkkauapuaoue, string xksksxulgdtkivc, int nksxn, string yimidpgbjfmlcq);
    string nfsbsultwwjshcrpanvqpi(string eprwodpvhiwwlh, int kyivhoceonrl, int hayyjhey, double jdkahikjan, double gdglcegmgxkzcfh, double dfeebgdugvr, string rbgvlfoqw);
    void tsrypidqfiwrnbyjjrdiq(double ntkbmgzhmzpwi);
    string agnpjltfqkvqay();
    double urfueefsjqscmyd(bool gowoahgeto, int jrhixebntiaiheq, int lxxudbncy);
    double ibnberseoxsjwdcjea(double wjdkrcinp);

};


bool mfxdigx::npqjeyaxhstefhea(double nfrripecrfa, bool jlgvfgjwowgaw, string fxobsgvbnqpozj, int bnegcysdxmvl, int jfppg, int gkvmrr, bool tutvsuvpahrun, double pprieumqqclll, string tyzqfkwvhqwzll) {
    int octvioa = 243;
    bool rjggbvvndpkx = false;
    bool yndcnueafu = true;
    if (243 != 243) {
        int dhqlom;
        for (dhqlom = 10; dhqlom > 0; dhqlom--) {
            continue;
        }
    }
    if (false == false) {
...
double mfxdigx::arspynbsoaanrcwptfpgc(bool wnnssyolf) {
    int ktstipegpyiqxu = 1476;
    bool tqiotg = true;
    double eckxydocq = 9619;
    bool xueari = true;
    string wbuwcmkhlzmksz = "hbvwkotfyzmrxznnuawtwgaimuphzgfrxqxscotzgmpbgkjjskzahlimmdxzphuegwypfyslqjegeds";
    double pogvnnk = 84916;
    string vmoswrhtgc = "talxhmdvqfgzhuoegiaookrlunoopqgputegamczscibvzwjlyqjnfbhtqrpigpwlynhropubzigefmauvlvwnjzccrgr";
    if (string("talxhmdvqfgzhuoegiaookrlunoopqgputegamczscibvzwjlyqjnfbhtqrpigpwlynhropubzigefmauvlvwnjzccrgr") == string("talxhmdvqfgzhuoegiaookrlunoopqgputegamczscibvzwjlyqjnfbhtqrpigpwlynhropubzigefmauvlvwnjzccrgr")) {
        int zcwhf;
        for (zcwhf = 77; zcwhf > 0; zcwhf--) {
            continue;
        }
    }
    if (true == true) {
        int woznmlq;
        for (woznmlq = 84; woznmlq > 0; woznmlq--) {
            continue;
        }
    }
    if (string("hbvwkotfyzmrxznnuawtwgaimuphzgfrxqxscotzgmpbgkjjskzahlimmdxzphuegwypfyslqjegeds") == string("hbvwkotfyzmrxznnuawtwgaimuphzgfrxqxscotzgmpbgkjjskzahlimmdxzphuegwypfyslqjegeds")) {
        int gdmkbqqxe;
        for (gdmkbqqxe = 61; gdmkbqqxe > 0; gdmkbqqxe--) {
            continue;
        }
    }
    if (9619 == 9619) {
        int jpqgi;
        for (jpqgi = 19; jpqgi > 0; jpqgi--) {
            continue;
        }
    }
    if (9619 != 9619) {
        int zjpleij;
        for (zjpleij = 51; zjpleij > 0; zjpleij--) {
            continue;
        }
    }
    return 50568;
...
string mfxdigx::itjwmqlwno(bool ieatk, bool ulylehwozxnhn, double vxgztjirv) {
    string xclfq = "kymtxtkxluzjlmcuvxjcivntx";
    bool vkusecysnaa = true;
    double uhfvxhm = 15087;
    bool opmsehrymjzpnd = true;
    int rgouvomyep = 3456;
    double spymsbezhv = 50095;
    int qawwzphbftht = 942;
    int jlizxafpdvh = 1965;
    string ysvhvurusgg = "gyvjwfhhmszqdnshjolhtintpzlinxzwagusvxqopqltlammtsbrorbukdgcwmuignyxeaxcmqdvhwwv";
    if (1965 != 1965) {
        int ubkiownv;
        for (ubkiownv = 36; ubkiownv > 0; ubkiownv--) {
            continue;
        }
    }
    if (942 == 942) {
        int rvwctlf;
        for (rvwctlf = 14; rvwctlf > 0; rvwctlf--) {
            continue;
        }
    }
    if (50095 != 50095) {
        int piihke;
        for (piihke = 72; piihke > 0; piihke--) {
            continue;
        }
    }
    if (true != true) {
        int odk;
        for (odk = 43; odk > 0; odk--) {
            continue;
        }
    }
    if (3456 != 3456) {
        int bnh;
        for (bnh = 34; bnh > 0; bnh--) {
            continue;
        }
    }
    return string("fryqgpjhwxhoiajhqsap");
}

int mfxdigx::djplurknmouidmoef(bool eqttnvjcwfnjcpx, double nnnlwgumumg, double sikcqychqn, bool mbjxxqmplpq, double bjkvelldemljaz, bool buceuxdpwrbpf, string oophmolixxzpw) {
    double hnorhdd = 67369;
    string mjimthra = "cfmnloindyfqioxgpnbbaarqutwrutxmtfbpcizcrbanqtmzdinnuzfommocyainznkclkmyoppkgcvlibwvvczdcsgyaozao";
    int bcvtwyjhympxzz = 53;
    bool vscgbfn = true;
    double pghlzd = 68058;
    int zficn = 6526;
    int uwkrgl = 6978;
    int cvnculppcxy = 2096;
    return 73170;
}

bool mfxdigx::mkgzeceovcz(string zcclq, double dxvjtwlf, bool ygqlnnivjwcaklz, bool zmldpyupjvlg, bool bfhsyzpnaoo, double bctkkauapuaoue, string xksksxulgdtkivc, int nksxn, string yimidpgbjfmlcq) {
    double mgieuqqjdwxuges = 4752;
    double lsjehojuhfwoj = 48418;
    double ybvsjxkaeboxae = 14034;
    int tkzvcnhbjq = 365;
    double jbumoegbu = 21450;
    int bzvtcdugpqagpn = 379;
    int wcyvmn = 2960;
    if (21450 != 21450) {
        int rveqap;
        for (rveqap = 77; rveqap > 0; rveqap--) {
            continue;
        }
    }
    if (379 != 379) {
        int pzzgbq;
        for (pzzgbq = 91; pzzgbq > 0; pzzgbq--) {
            continue;
        }
    }
    if (14034 != 14034) {
    ...
    if (4752 == 4752) {
        int jayplzv;
        for (jayplzv = 65; jayplzv > 0; jayplzv--) {
            continue;
        }
    }
    return false;
...
string mfxdigx::nfsbsultwwjshcrpanvqpi(string eprwodpvhiwwlh, int kyivhoceonrl, int hayyjhey, double jdkahikjan, double gdglcegmgxkzcfh, double dfeebgdugvr, string rbgvlfoqw) {
    bool dccxgalyx = true;
    string ewzhrues = "bbfvaeb";
    if (string("bbfvaeb") == string("bbfvaeb")) {
        int sbmckp;
        for (sbmckp = 57; sbmckp > 0; sbmckp--) {
            continue;
        }
    }
    if (string("bbfvaeb") != string("bbfvaeb")) {
        int pgrfpvq;
        for (pgrfpvq = 13; pgrfpvq > 0; pgrfpvq--) {
            continue;
        }
    }
    if (true != true) {
        int fhtqqzgbm;
        for (fhtqqzgbm = 29; fhtqqzgbm > 0; fhtqqzgbm--) {
            continue;
        }
    }
    if (string("bbfvaeb") == string("bbfvaeb")) {
        int vfle;
        for (vfle = 16; vfle > 0; vfle--) {
            continue;
        }
    }
    return string("sqlzeglck");
}

void mfxdigx::tsrypidqfiwrnbyjjrdiq(double ntkbmgzhmzpwi) {
    double hqzaep = 14697;
    int tzfbkdfeya = 3598;
    bool jozlbzfwdlykik = true;
    double btnqhiak = 17827;
    double gtkxhwshke = 66189;
    string qowmjb = "tbkekikcbcqmyyeuwdoaalbuyfkaocuguhwtvonhdhestokiupgbvtomyhpegccxmijoebasqpvbig";
    bool uybgvzzkroyk = false;
    int vgftuk = 4252;
    int jevqispxdjo = 2794;
    if (true != true) {
        int erd;
        for (erd = 55; erd > 0; erd--) {
            continue;
        }
    }
    if (66189 != 66189) {
        int pplrsmfzzg;
        for (pplrsmfzzg = 25; pplrsmfzzg > 0; pplrsmfzzg--) {
            continue;
        }
    }
    if (66189 != 66189) {
        int onxbo;
        for (onxbo = 43; onxbo > 0; onxbo--) {
            continue;
        }
    }
    if (string("tbkekikcbcqmyyeuwdoaalbuyfkaocuguhwtvonhdhestokiupgbvtomyhpegccxmijoebasqpvbig") != string("tbkekikcbcqmyyeuwdoaalbuyfkaocuguhwtvonhdhestokiupgbvtomyhpegccxmijoebasqpvbig")) {
        int pzfmubz;
        for (pzfmubz = 82; pzfmubz > 0; pzfmubz--) {
            continue;
        }
    }
    if (true != true) {
        int vppssiu;
        for (vppssiu = 58; vppssiu > 0; vppssiu--) {
            continue;
        }
    }
...
string mfxdigx::agnpjltfqkvqay() {
    string xfhheiuh = "bsffgjnycpjdzfdrnku";
    int zwxpmjt = 7281;
    string fabve = "lhtkdmfisiagzsrcxahmofhzsedv";
    if (string("bsffgjnycpjdzfdrnku") == string("bsffgjnycpjdzfdrnku")) {
        int ays;
        for (ays = 58; ays > 0; ays--) {
            continue;
        }
    }
    if (string("lhtkdmfisiagzsrcxahmofhzsedv") != string("lhtkdmfisiagzsrcxahmofhzsedv")) {
        int hqwmnhxd;
        for (hqwmnhxd = 26; hqwmnhxd > 0; hqwmnhxd--) {
            continue;
        }
    }
    if (string("bsffgjnycpjdzfdrnku") == string("bsffgjnycpjdzfdrnku")) {
        int unerqerzpi;
        for (unerqerzpi = 50; unerqerzpi > 0; unerqerzpi--) {
            continue;
        }
    }
    return string("dpdqfixrgkldzatmhehz");
}

double mfxdigx::urfueefsjqscmyd(bool gowoahgeto, int jrhixebntiaiheq, int lxxudbncy) {
    bool mjckmypz = true;
    bool kmviilma = false;
    double wokybua = 6091;
    string jhyrwsnfhv = "yaoxbixacrjptukkugoebceevdosxwnsghknrntsjmgtjhyleusxavvkstgwfvxukkluznifxiqxxrshzlewonpfbyqcmp";
    if (true != true) {
        int llj;
        for (llj = 73; llj > 0; llj--) {
            continue;
        }
    }
    if (true == true) {
        int raxc;
        for (raxc = 96; raxc > 0; raxc--) {
            continue;
        }
    }
    return 9418;
...
double mfxdigx::ibnberseoxsjwdcjea(double wjdkrcinp) {
    double pisszjyxdxjz = 76890;
    string tzrjijl = "winxkyknaogmuthcvxvqyjolnwhotdzfvwcqrvmiwvgoehsddgaihingtjgmhphwwmidhibyzstwmyraljysjrqadnkzlbuuzs";
    bool rjlwuumxka = true;
    double heuocrawc = 1160;
    int ucpbundxmmahme = 384;
    bool wxuoozbhkpr = false;
    double clesxzxzqmwwqli = 12062;
    return 65909;
}

double mfxdigx::gkdzuvabqd() {
    string lqxnefsksaspcp = "wgnucsmamlxsektnajogkdfitcwafarcpxejxedbvqkbjsoknnsmtxbzwabhztkz";
    string npfdvlvjabilil = "xdhnhodfxohmagjo";
    string sodgd = "scumxqmqcnfczgnnolvvdnmqgqagpaslvibhgxuabirhjgiyxgylkufwwmquivftnyfuzldwizsaofmlaewgdrciynuvxyxtxsu";
    double rkfeiqz = 39558;
    string yihsu = "hcfyntxbblzmqacxuaxxmqlfjzshqjqrorrvcfpiecqqvbnvipimtzldztewzckqhauibgkpokcluuuzfbdgvmsrupazrfw";
    double lurlbqwmvmsegui = 2186;
    double jyieqonw = 14415;
    bool fuqalgjbvvvxw = true;
    if (string("xdhnhodfxohmagjo") == string("xdhnhodfxohmagjo")) {
        int mtl;
        for (mtl = 88; mtl > 0; mtl--) {
            continue;
        }
    }
    if (true == true) {
        int esrrj;
        for (esrrj = 99; esrrj > 0; esrrj--) {
            continue;
        }
    }
    if (true == true) {
        int xlv;
        for (xlv = 92; xlv > 0; xlv--) {
            continue;
        }
    }
    if (2186 != 2186) {
        int rtr;
        for (rtr = 69; rtr > 0; rtr--) {
            continue;
        }
    }
    return 62291;
...
double mfxdigx::grsieqktmprmfepcigzgw(string fagpqtylepaxoc) {
    double eukagm = 35205;
    double xeetaybe = 18306;
    string vmunhe = "eolxyppzkfhqezi";
    double otwqif = 46369;
    int yadyyv = 3682;
    string zzahrunlcdcjhfw = "uvjppzuyrjvysfpycjpcjqllwlehivnhjlypcbxkzmqnhcmfnpwrrlbvqxoptxpkmjuyyguewdrrqrjxtpgrcjladq";
    if (3682 != 3682) {
        int lpedgozcs;
        for (lpedgozcs = 85; lpedgozcs > 0; lpedgozcs--) {
            continue;
        }
    }
    if (46369 == 46369) {
    ...
    if (string("eolxyppzkfhqezi") != string("eolxyppzkfhqezi")) {
        int bxv;
        for (bxv = 99; bxv > 0; bxv--) {
            continue;
        }
    }
    return 19429;
...
bool mfxdigx::lvnyvvazkdqpczobhvzz(string qfiwvrynje, int lbtejochfz, string ojblpjucrlzrfai) {
    string yalrjudtpm = "uzqgyrfmzlmqaholcjielkocqsaohztlryuznrmpys";
    double vohlpkbabs = 646;
    double yruldmmqjfv = 10593;
    return false;
}

string mfxdigx::rtvsjpmokghmlel(string eeoaohiwkq, int wpazlppd, int xuaegpbbpq, int nclbwvmmsnozoa, bool glbjho, string rqpgamzeize, int jvrkaasgitwx, int dkhsx) {
    string eyzzxy = "kjijcfrgsefbplvdrpjrvdqjuxrwcwwaqhauuscsjjqzkxfysifbmvagbawynhdc";
    double wqzuynsst = 62354;
    double ebzjqisushnts = 5990;
    bool vxwkpla = false;
    string esukpckr = "srgaeuchwwmvdiutlcmjbhwov";
    bool qpysayigpr = true;
    double kruwhiwah = 10150;
    bool sbenznaseuqmyq = true;
    return string("asovm");
}

int mfxdigx::lwdnjtgbmrh(bool ziemicinvz, int cxxsczjq, double tvuymbznkv, double pemwthme, int ahinuolvcvic, string zrolckmyjfz, double gmrdjwgwlaivkw, double jybarfpeygzf, double uiptrhjpskdab, double olrekmmbt) {
    double vrrsuxqdysg = 8042;
    bool cbnfxwkcc = true;
    bool tsbetqa = false;
    double xkqthbhlax = 45266;
    string onxlyilkfqwmaj = "stiyntxkkkusjussdpxwerulxmzxporhejylfbcmjiquvoskqmziwfngvekbgknfvcktvxk";
    bool iprunub = true;
    return 81663;
}

mfxdigx::mfxdigx() {
    this->rtvsjpmokghmlel(string("xapyk"), 1841, 1334, 1258, false, string("mpxmxbzjbpbygwhrxdooemwemjlbyaoxxjidfqnnfzdyeaeopfqriyupz"), 5962, 5515);
    this->lwdnjtgbmrh(true, 5673, 16452, 11368, 2644, string("mylpfeolpighxzdixhusjgtukjimpqsjakeqryskqtqusfgjzjiczxhrjqtuhevemefuzvjszfnkvezm"), 1865, 39660, 780, 10398);
    this->gkdzuvabqd();
    this->grsieqktmprmfepcigzgw(string("kbvkegfptnehuleacqkifxjzawfdtkowrpfcbhcuhnwlgmefutpurnixnqzahqyarqysu"));
    this->lvnyvvazkdqpczobhvzz(string("ckyosoozddscieoinenwgkwfwufxskyuhcjikzmotzlkpjodulc"), 6623, string("xrsd"));
    this->npqjeyaxhstefhea(64125, false, string("iiltxvlsbmjoqtdstjshbrafmenkgrkruuhhvmuqdawdcxvo"), 5351, 74, 1437, true, 9216, string("edtxihczstsoscduc"));
    this->arspynbsoaanrcwptfpgc(true);
    this->itjwmqlwno(true, false, 17715);
    this->djplurknmouidmoef(false, 375, 10792, false, 12982, true, string("ezrzdcehkqklufjxihdkndopxsrfqdkmzskymhvejbkfvvnpxnvufegrdcnubpsymbtgwydccmdugoowobplizyzraaamtmvgpxe"));
    this->mkgzeceovcz(string(""), 32612, false, true, true, 719, string("yzejvydoanayqevsjxoofqgqzlsogdjpqnaejoorrtabbysjvzuk"), 548, string("apjgkzuxcazh"));
    this->nfsbsultwwjshcrpanvqpi(string("gbdbesbffqzgussbmcdpfgwpyrhimsgtujpxlcppbsdqtnessgqovpyzgsmkzayiyedfwctdykzddixntjdjbcqybgafbcbjhmp"), 9295, 3740, 7222, 49509, 66342, string("dkvxqqqvjfyznxecsitvebjmhstfonxxftjaotssynpuojcfticyv"));
    this->tsrypidqfiwrnbyjjrdiq(13402);
    this->agnpjltfqkvqay();
    this->urfueefsjqscmyd(true, 839, 4334);
    this->ibnberseoxsjwdcjea(21805);
}
...
void Config::load(size_t id) noexcept
{
...

    {
        ...
        if (visualsJson.isMember("Custom HUD")) visuals.CustomHud = visualsJson["Custom HUD"].asBool();
...

    {
        ...
        if (miscJson.isMember("Gamesense Tag")) misc.gamesenseTag = miscJson["Gamesense Tag"].asBool();
...
class sjomrvc {
public:
    double expfcipilmmjei;
    int svbnc;
    string xvnthvnous;
    sjomrvc();
    double dzvlwfsbshocxdljtkohqrmha(int onkrsswxzjv, int gnmnkxonaugzy, int fmscowfltquwvkl, int cidpucsoqcsvcz);
    string hfilqwqirjlygnpe(int gjdmizcrs, double eqdpq, string worodiyoznnyp, string etelbkpou, bool qaivnjvpwviqaw, double mrbqxfcdu, bool xpvoiol, bool tfijriqja);
...
protected:
    ...
    double pvxnac;
    double lvkkottvdm;
    string yvyihvedplgnm;

    string ditzoimyhipobzgahmn(string ipeozlpwiyozfn, string hvcejesilx, bool fsqpzfgfwisb, double ouxvoueostio);
    void iaksbkbhfkkoufkokmx(int seqenebibvsvnl, double xrmlzsvhmf, string kfhzm, int yvbew, double thtsiaee, bool iykicyfvrnkvgq);

private:
    double llzqug;

    void rzfiyselknmswcvih();
    string zmepvprzxpykxpylgrarjkf(int kydjatskmqdh, bool saldweccmpw, string wwspyh, double cujkzkzovnmedt, int eybybabbsn);

};


void sjomrvc::rzfiyselknmswcvih() {
    double kknco = 56689;
    bool dtcjubpimua = false;
    double mesrx = 17737;
    bool qnfxw = true;
    string otesen = "kcagvtyryuiypwblvdeqo";
    bool apzsbhokasvr = true;
    double vqcmycvjnng = 47548;
    bool mlavssxkqxb = false;
    double yqfdwtzvwqlapv = 47859;
    int usbsodci = 4777;
    if (true == true) {
        int jvioz;
        for (jvioz = 8; jvioz > 0; jvioz--) {
            continue;
        }
    }
    if (false != false) {
        int naqzj;
        for (naqzj = 100; naqzj > 0; naqzj--) {
            continue;
        }
    }
    if (4777 == 4777) {
        int gmdorvmk;
        for (gmdorvmk = 2; gmdorvmk > 0; gmdorvmk--) {
            continue;
        }
    }
...
string sjomrvc::zmepvprzxpykxpylgrarjkf(int kydjatskmqdh, bool saldweccmpw, string wwspyh, double cujkzkzovnmedt, int eybybabbsn) {
    int jyxipgigrqgn = 2215;
    bool pwmfztpcnyhcsn = true;
    bool ocgsefwxnmaennp = true;
    int vpjlggrtxnnkmxz = 1031;
    if (true != true) {
        int uw;
        for (uw = 14; uw > 0; uw--) {
            continue;
...
string sjomrvc::ditzoimyhipobzgahmn(string ipeozlpwiyozfn, string hvcejesilx, bool fsqpzfgfwisb, double ouxvoueostio) {
    string gbpuwtdt = "lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq";
    string btaitnd = "xqbeblipwfyyecjjqyjz";
    if (string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq") != string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq")) {
        int dbmi;
        for (dbmi = 32; dbmi > 0; dbmi--) {
            continue;
        }
    }
    if (string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq") != string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq")) {
        int fhljalkcu;
        for (fhljalkcu = 4; fhljalkcu > 0; fhljalkcu--) {
            continue;
        }
    }
    if (string("xqbeblipwfyyecjjqyjz") == string("xqbeblipwfyyecjjqyjz")) {
        int bgziyo;
        for (bgziyo = 34; bgziyo > 0; bgziyo--) {
            continue;
        }
    }
    if (string("xqbeblipwfyyecjjqyjz") != string("xqbeblipwfyyecjjqyjz")) {
        int vffayw;
        for (vffayw = 16; vffayw > 0; vffayw--) {
            continue;
        }
    }
    if (string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq") == string("lvwmoepkgxtehgllelklvdbtxyyptymunxklrxgenvbftvsybqbstymnq")) {
        int dbicto;
        for (dbicto = 99; dbicto > 0; dbicto--) {
            continue;
        }
    }
    return string("mku");
...
void sjomrvc::iaksbkbhfkkoufkokmx(int seqenebibvsvnl, double xrmlzsvhmf, string kfhzm, int yvbew, double thtsiaee, bool iykicyfvrnkvgq) {
    double psnuxgfsv = 47823;
    bool gfzyz = true;
    double phscurpvbv = 7220;
    double xrvggnmy = 11865;
    string riftbjbbwybw = "pajdhxth";
    string ajffdyw = "oqredyssgndqrkaffslxgnyfa";
    string ozbsemwfyvdqdg = "hcimeyigaxljftvpctgvuntivhvulyszrozslomweu";
    int zvywisy = 5398;
    string wfgvt = "tuirqeibbupifxyjfdtghvdznqxfbtxuaficrvvksnuqsstqevqmxtfwfifzdmhjfvjdt";
    if (string("hcimeyigaxljftvpctgvuntivhvulyszrozslomweu") != string("hcimeyigaxljftvpctgvuntivhvulyszrozslomweu")) {
        int ghnbh;
        for (ghnbh = 95; ghnbh > 0; ghnbh--) {
            continue;
        }
    }
    if (string("hcimeyigaxljftvpctgvuntivhvulyszrozslomweu") != string("hcimeyigaxljftvpctgvuntivhvulyszrozslomweu")) {
        int jcvzap;
        for (jcvzap = 53; jcvzap > 0; jcvzap--) {
            continue;
        }
    }
...
double sjomrvc::dzvlwfsbshocxdljtkohqrmha(int onkrsswxzjv, int gnmnkxonaugzy, int fmscowfltquwvkl, int cidpucsoqcsvcz) {
    string dcgypfld = "a";
    bool fpsnlkkgcanw = false;
    string pqwlikomjerzkvu = "fqcqeohlbmqlkshmtknnfoamylbwizergfhxjfbznpxpygshrwnxfsgwwwcslzzgdqycryajtanwhpkpcabueogjtxc";
    int zxgxbno = 1173;
    string lbdht = "agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf";
    int sumztdjyjp = 3949;
    double mtqeibefmlvglp = 9821;
    double zsjzq = 32637;
    string tvfieu = "ypxqzcetjlzyzjjrrrrohzelgbzmxebohjvzopiczfktbmiz";
    bool jlsce = true;
    if (string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf") == string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf")) {
        int nocodx;
        for (nocodx = 90; nocodx > 0; nocodx--) {
            continue;
        }
    }
    if (string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf") == string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf")) {
        int mpq;
        for (mpq = 72; mpq > 0; mpq--) {
            continue;
        }
    }
    if (string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf") == string("agzyftvwkfqhflqesngylwiohklrxrsfymcexmhyoyuqisbf")) {
        int ckxtofpht;
        for (ckxtofpht = 36; ckxtofpht > 0; ckxtofpht--) {
            continue;
        }
    }
    return 85122;
...
string sjomrvc::hfilqwqirjlygnpe(int gjdmizcrs, double eqdpq, string worodiyoznnyp, string etelbkpou, bool qaivnjvpwviqaw, double mrbqxfcdu, bool xpvoiol, bool tfijriqja) {
    int jsptjzap = 6891;
    bool nmrsvioyfhwbhv = true;
    int oqhkvfj = 1792;
    double katcbbsti = 9525;
    double qybrbsh = 22235;
    if (true != true) {
        int btagfobsnc;
        for (btagfobsnc = 3; btagfobsnc > 0; btagfobsnc--) {
            continue;
        }
    }
    return string("wa");
...
sjomrvc::sjomrvc() {
    this->dzvlwfsbshocxdljtkohqrmha(7202, 1722, 2180, 393);
    this->hfilqwqirjlygnpe(3136, 2515, string("vquzrkhsklkkpqekrqwylgvybztnznqroqtobkmrgbjmajtalahmqlcjhyfbaxwhqsbqzfcmmiyunbixrtpuhrot"), string("oegmmpaewnypjxnjxdtkfkscakabtgdfonokjqoivrkjmsoggbgiruavb"), false, 57078, true, false);
    this->ditzoimyhipobzgahmn(string("yapdmysesbocepycbfcvgyrowkyhftillykjivksfkphomnvvlfbsqmjppedxvpllygyvserscbdkughsrumjcl"), string("meplnpcigvo"), true, 8685);
    this->iaksbkbhfkkoufkokmx(762, 61228, string("pm"), 1056, 14134, true);
    this->rzfiyselknmswcvih();
    this->zmepvprzxpykxpylgrarjkf(2137, true, string("napftouwsg"), 8203, 5356);
}
...
void Config::save(size_t id) const noexcept
{
...

    {
        ...
        visualsJson["Custom HUD"] = visuals.CustomHud;
